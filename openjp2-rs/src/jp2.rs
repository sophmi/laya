...
impl Jp2BoxHeader {
    fn new(ty: Jp2BoxType) -> Self {
        Self { length: 8, ty, header_length: 8 }
    }
    ...
    fn read<R: Read + ?Sized>(&mut self, reader: &mut R, max_size: usize) -> Result<(), String> {
        ...
        if self.length == 0 {
            if max_size == 0 {
                return Err("Can't handle box of undefined size.".to_string());
            }
            // last box
            if max_size > u32::MAX as usize {
                // TODO: Handle large boxes?
                return Err("Cannot handle box sizes higher than 2^32".to_string());
            }
...
impl HeaderWriter {
    fn new(handler: fn(_: &mut opj_jp2, _: &mut Vec<u8>) -> bool) -> Self {
        Self { handler, data: Default::default() }
    }
...
fn opj_jp2_read_ihdr(jp2: &mut opj_jp2, mut buf: &[u8], p_manager: &mut opj_event_mgr) -> OPJ_BOOL {
    ...
    if numcomps > 16384 {
        ...
        event_msg!(p_manager, EVT_ERROR, "Invalid number of components (ihdr)\n",);
        return 0;
    }
...
fn opj_jp2_write_cdef(mut jp2: &mut opj_jp2, buf: &mut Vec<u8>) -> bool {
    ...
    for info in info {
        buf.write_u16::<BigEndian>(info.cn).unwrap();
        buf.write_u16::<BigEndian>(info.typ).unwrap();
        buf.write_u16::<BigEndian>(info.asoc).unwrap();
    }
    true
}
...
fn opj_jp2_write_colr(mut jp2: &mut opj_jp2, buf: &mut Vec<u8>) -> bool {
    ...
    match meth {
        1 => {
            // EnumCS
            buf.write_u32::<BigEndian>(jp2.enumcs).unwrap();
...
fn opj_jp2_check_color(
    ...
    if let Some(cdef) = &color.jp2_cdef {
        ...
        for chan in 1..nr_channels {
            let has = cdef.info.iter().any(|&x| x.cn as u32 == chan);
            if !has {
...
fn opj_jp2_read_pclr(jp2: &mut opj_jp2, mut buf: &[u8], p_manager: &mut opj_event_mgr) -> OPJ_BOOL {
    ...
    let jp2_pclr =
        opj_jp2_pclr { channel, entries, nr_entries, nr_channels, cmap: Default::default() };
        ...
        if cn as u32 >= numcomps {
            ...
            if acn as u32 >= numcomps {
                ...
                if cn != acn && info.typ == 0 {
                    ...
                    std::mem::swap(&mut cn_comp, &mut (*acn_comp));
                    // Swap channels in following channel definitions, don't bother with j <= i that
...
// *
// Writes the Jpeg2000 codestream Header box - JP2C Header box. This function must be called AFTER
// the coding has been done.
//
// @param   stream         the stream to write data to.
// @param   jp2         the jpeg2000 file codec.
// @param   p_manager   user event manager.
...
fn opj_jp2_write_jp2c(
    ...
    if !header.write(stream) {
        event_msg!(p_manager, EVT_ERROR, "Error while writing jp2c header to stream\n",);
        return 0;
    }
...
fn opj_jp2_write_jp(
    ...
    if stream.write_all(&tmp_buf).is_err() {
        event_msg!(p_manager, EVT_ERROR, "Error while writing jp data to stream\n",);
        return 0;
    }
    1
}
...
pub(crate) fn opj_jp2_setup_encoder(
    ...
    if alpha_count == 1 {
        ...
        let mut cdef = opj_jp2_cdef { info: Vec::with_capacity(len) };
        // no memset needed, all values will be overwritten except if jp2->color.jp2_cdef->info
        // allocation fails,
        ...
        for i in 0..len {
            ...
            if i < color_channels as usize {
                cdef.info
                    .push(opj_jp2_cdef_info { cn, typ: 0, asoc: cn + 1 })
            } else if comps[i].alpha != 0 {
                ...
                // Unknown channel
                cdef.info
                    .push(opj_jp2_cdef_info { cn, typ: u16::MAX, asoc: u16::MAX })
            }
        }
...
fn opj_jp2_default_validation(
    ...
    l_is_valid &= (!jp2.cl.is_empty()) as core::ffi::c_int;
    // width
        ...
        if header.ty.is_file_header() || header.ty.is_image_header() {
            if !header.ty.is_file_header() {
                ...
                if jp2.jp2_state & JP2_STATE_HEADER != 0 {
                    ...
                    if opj_stream_skip(stream, data_size as OPJ_OFF_T, p_manager)
                        != data_size as i64
                    {
                        event_msg!(
                            p_manager,
...
// *
// Sets up the procedures to do on writing header. Developers wanting to extend the library can add
// their own writing procedures.
