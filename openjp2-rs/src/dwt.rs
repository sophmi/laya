        ...
        if self.cas == 0 {
            if self.dn > 0 || self.sn > 1 {
                ...
                for i in win_h_x0..win_h_x1 {
                    ...
                    for off in 0..4 {
                        let val = self.m(d_idx + off).wrapping_add(
                            self.m(s0_idx + off).wrapping_add(self.m(s1_idx + off)) >> 1,
                        );
...
// </summary>
unsafe fn opj_dwt_interleave_h(mut h: &opj_dwt_t, mut a: *mut OPJ_INT32) {
...
// </summary>
unsafe fn opj_dwt_interleave_v(mut v: &opj_dwt_t, mut a: *mut OPJ_INT32, mut x: OPJ_INT32) {
    let mut ai: *const OPJ_INT32 = a;
...
unsafe fn opj_v8dwt_encode_step2(
    ...
    if imax > 0u32 {
        ...
        while c < NB_ELTS_V8 as i32 {
            ...
            *fresh47 += (*fl.offset(c as isize) + *fw.offset(c as isize)) * cst;
            c += 1
        }
...
unsafe extern "C" fn opj_dwt_encode_and_deinterleave_v_real(
    ...
    opj_v8dwt_encode_step1(tmp.offset((b * NB_ELTS_V8 as i32) as isize), dn as OPJ_UINT32, opj_K);
...
unsafe fn opj_dwt_encode_procedure(
    ...
    loop {
        ...
        if fresh50 == 0 {
            break; // 0 = non inversion on horizontal filtering 1 = inversion between low-pass and
                   // high-pass filtering
...
unsafe fn opj_dwt_decode_tile(
    ...
    let mut h = opj_dwt_t { mem: std::ptr::null_mut::<OPJ_INT32>(), dn: 0, sn: 0, cas: 0 }; // width of the resolution level computed
    let mut v = opj_dwt_t { mem: std::ptr::null_mut::<OPJ_INT32>(), dn: 0, sn: 0, cas: 0 }; // height of the resolution level computed
...
unsafe fn opj_dwt_interleave_partial_v(
    ...
    ret = opj_sparse_array_int32_read(
        ...
        dest.offset(((1i32 - cas) * 4i32) as isize)
            ...
            while precno < (*res).pw.wrapping_mul((*res).ph) {
                ...
                while cblkno < (*precinct).cw.wrapping_mul((*precinct).ch) {
                    let mut cblk: *mut opj_tcd_cblk_dec_t =
                        &mut *(*precinct).cblks.dec.offset(cblkno as isize)
                            as *mut opj_tcd_cblk_dec_t;
...
unsafe fn opj_dwt_decode_partial_tile(
    ...
    let mut h = opj_dwt_t { mem: std::ptr::null_mut::<OPJ_INT32>(), dn: 0, sn: 0, cas: 0 };
    let mut v = opj_dwt_t { mem: std::ptr::null_mut::<OPJ_INT32>(), dn: 0, sn: 0, cas: 0 };
    let mut resno: OPJ_UINT32 = 0;
    ...
    while resno < numres {
        ...
        opj_dwt_segment_grow(filter_width, h.sn as OPJ_UINT32, &mut win_ll_x0, &mut win_ll_x1);
        opj_dwt_segment_grow(filter_width, h.dn as OPJ_UINT32, &mut win_hl_x0, &mut win_hl_x1);
        opj_dwt_segment_grow(filter_width, v.sn as OPJ_UINT32, &mut win_ll_y0, &mut win_ll_y1);
        opj_dwt_segment_grow(filter_width, v.dn as OPJ_UINT32, &mut win_lh_y0, &mut win_lh_y1);
        ...
        while j < rh {
                    ...
                    && j < win_lh_y1.wrapping_add(v.sn as OPJ_UINT32)
            {
                ...
                // signed integer overflow: -1094795586 + -1094795586 cannot be represented in type
                // 'int'
...
// </summary>
unsafe fn opj_v8dwt_decode(mut dwt: &opj_v8dwt_t) {
    let mut a: OPJ_INT32 = 0;
    let mut b: OPJ_INT32 = 0;
    ...
    opj_v8dwt_decode_step1(dwt.wavelet.offset(a as isize), dwt.win_l_x0, dwt.win_l_x1, opj_K);
    opj_v8dwt_decode_step1(dwt.wavelet.offset(b as isize), dwt.win_h_x0, dwt.win_h_x1, two_invK);
            ...
            while k_1 < rh {
                memcpy(
                    &mut *aj.add((k_1 as usize).wrapping_mul(w as OPJ_SIZE_T)) as *mut OPJ_FLOAT32
                        as *mut core::ffi::c_void,
                    &mut *v.wavelet.offset(k_1 as isize) as *mut opj_v8_t
                        as *const core::ffi::c_void,
